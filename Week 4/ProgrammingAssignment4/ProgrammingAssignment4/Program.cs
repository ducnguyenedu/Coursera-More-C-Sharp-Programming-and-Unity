    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                // for every chars occuring countings
                int[] counting = new int[26];

                // initializing every index with 0
                for (int i = 0; i < 26; i++)
                {
                    counting[i] = 0;
                }

                // for increasing occurence couning
                for (int i = 0; i < input.Length; i++)
                {
                    int index = ConvertCharToIndex(input[i]);
                    if (index != -1)
                    {
                        counting[index] += 1;
                    }
                ...
                for (int i = 0; i < 26; i++)
                {
                    if (counting[i] != 0)
                    {
                        Console.Write($"{ConvertIndexToChar(i)}{counting[i]} ");
                    }
                }
